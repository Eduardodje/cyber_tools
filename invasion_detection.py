...
def detectar_invasao(registros):
    ...
    for registro in registros:
        ...
        if id_usuario == usuario_atual:
            if status == "falha":
                # Se for falha, incrementa o contador de falhas consecutivas
                tentativas_consecutivas += 1
            else:
                # Se for sucesso, reseta o contador de falhas
                tentativas_consecutivas = 0
        else:
            # Se mudar de usuário, verifica se o anterior teve mais de 3 falhas consecutivas
...
def main():
    ...
    # Divide a entrada em registros individuais
